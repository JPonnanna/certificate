...
def conclude(prompt):
    url = "https://a-207362899381.asia-south1.run.app/a7f9c3d2b1"
    ...
    try:
        ...
        # Ensure it always returns the expected format
...
def verify_certificate_cloud(actual_image_path, original_image_path):
    """
    Send two certificate images to cloud for comparison
    
    Args:
        actual_image_path: Path to uploaded certificate image
        original_image_path: Path to QR-downloaded certificate image
    
    Returns:
        dict: Response from cloud with verification result
    """
    url = "https://a-207362899381.asia-south1.run.app/compare_certificates"
    
    try:
        # Open and send both images
        with open(actual_image_path, 'rb') as actual_file, \
             open(original_image_path, 'rb') as original_file:
            
            files = {
                'actual': ('actual.jpg', actual_file, 'image/jpeg'),
                'original': ('original.jpg', original_file, 'image/jpeg')
            }
...
def qr_extract(filename):
    ...
    if result['status'] == 'success':
        ...
        prefix = "https://cyan-magnificent-moose-511.mypinata.cloud"
        if prefix not in result['url']:
            mes=f"❌ QR Extraction Failed: Conflicting QRs Found/ Multiple QRs Found"
...
def compare_certificates():
    ...
    # Get image paths
    base_name = uploaded_pdf.rsplit('.', 1)[0]
    actual_image_path = os.path.join(app.config['UPLOAD_FOLDER'], f"converted_{base_name}.jpg")
    original_image_path = os.path.join(app.config['UPLOAD_FOLDER'], f"converted_{base_name}_qr.jpg")
    
    # Send to cloud for processing
    cloud_result = verify_certificate_cloud(actual_image_path, original_image_path)
    
    if cloud_result['status'] == 'success':
        result = cloud_result['result'].get('verification_result', '')
        return render_template("result.html", result=result, uploaded_pdf=uploaded_pdf, fetched_pdf_url=fetched_pdf_url)
    else:
        return render_template('failedqr.html', message=f"❌ Verification Failed: {cloud_result['error']}")
...
#     # result=trained_llama('',verified)
#     #print(result)
#     result = conclude(compare_prompt)
#     uploaded_pdf = session.get('uploaded_pdf')
#     fetched_pdf_url = session.get('fetched_pdf_link')
#     return render_template("result.html", result=result, uploaded_pdf=uploaded_pdf, fetched_pdf_url=fetched_pdf_url)
